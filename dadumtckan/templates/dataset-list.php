			...
			?>
					...
					<?php
						...
						if ($should_link_to_dataset): ?>
							<a <?php if ($target_blank_enabled){ echo 'target="_blank"';} ?>  href="<?php echo of_get_option('datasetsres_link').'&id='.$dataset["name"].'&title='.$dataset["title"] ?>">
								<?php echo $to_print; ?>
							</a>
						<?php else: ?>
							<?php echo $to_print; ?>
			...
			<?php if (array_key_exists("resources",$dataset) && !empty($include_fields_resources)){ ?>
				<div class="wpckan_resources_list">
					...
					<div class="wpckan_resource">
						<?php foreach ($include_fields_resources as $field_name):
							...
							if ($should_link_to_dataset): ?>
								<a <?php if ($target_blank_enabled){ echo 'target="_blank"';} ?>  href="<?php echo wpckan_get_link_to_resource($dataset["name"],$resource["id"]) ?>">
									<?php echo $to_print; ?>
								</a>
							<?php else: ?>
								<?php echo $to_print; ?>
...
<div class="wpckan_dataset_list_pagination">
	<?php if (array_key_exists("limit",$atts) && array_key_exists("page",$atts) && array_key_exists("prev_page_link",$atts)){
	...
	?>
	<?php if (array_key_exists("limit",$atts) && array_key_exists("page",$atts) && array_key_exists("next_page_link",$atts)){
